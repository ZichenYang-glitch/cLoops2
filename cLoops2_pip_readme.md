...
`-tot` is the target depth of the complete retained library; `-mode` only
chooses the physical projection written from that same global sample.

| `-mode` | sampled universe | physical output | `Unique PETs` | represented depth |
|---|---|---|---:|---:|
...
Without `pre -trans`, `cis` and `all` are equivalent because trans PETs were
not retained. A shared trans workflow must use `pre -trans` and then
`samplePETs -mode all`.

`-tot` below/equal to/above the source logical depth selects exact global
without-replacement sampling, identity, or replacement sampling. Replacement
output is descriptive only and formal p/q-value callers reject it. A fixed
`-seed` is reproducible independently of file-worker scheduling.

Root and fully materialized `all` inputs can be upsampled. A cis/trans
projection currently supports only same-mode nested downsampling; return to
the original `pre` or an `-mode all` parent for a larger target.
...
`petMeta.json` keeps physical `Unique PETs` separate from
`Sampling.target_logical_total`, records cis/trans allocations, per-record
counts, RNG/seed and replacement ancestry. The full algorithm and downstream
contract are in `trans_downstream_adaptation_plan.md`.

Older peak/domain, agg, montage and viewpoint paths are not all projection
aware. Prefer `pre -trans → samplePETs -mode all`, where physical `Unique
PETs` equals logical `Lglobal`, for a shared downstream workflow.

Combined directories retain structural depth/replacement metadata but are not
automatically eligible for formal p/q-value analysis. Use a current multi-file
`pre` root or per-sample raw inputs for inferential branches; legacy combine
directories without `Retention` remain exploratory.
...
Trans analysis has two separate tracks. `-mode trans` without fixed candidates
writes exploratory `*_trans_candidates.txt` with adjusted/significant fields
set to `NA`. Supplying an independent family with
`-trans_candidates/-trans_candidate_source` performs strict directional
global/local tests and writes `*_trans_loops.txt`; this fixed trans-only path
does not require `-eps/-minPts`. Legacy `-trans` maps to `-mode all`.
For de novo formal inference, `-trans_split -trans_split_seed INT` makes
disjoint discovery/validation PET partitions: DBSCAN only sees discovery and
validation supplies every inferential count and p-value.

```bash
cLoops2 callLoops -d sample.all -o sample -mode trans \
  -eps 5000 -minPts 10
cLoops2 callLoops -d sample.all -o sample.fixed -mode trans \
...
cLoops2 callLoops -d sample.all -o sample.split -mode trans \
  -eps 5000 -minPts 20 -trans_split -trans_split_seed 123
cLoops2 quant -d sample.all -loops preregistered_trans.tsv \
  -o sample.quant -mode trans
cLoops2 dump -d sample.all -o chr1_chr2 -trans_mat \
  -mat_chrom chr1-chr2 -trans_mat_method pair_oe
cLoops2 dump -d sample.all -o sample.trans -bedpe -mode trans
cLoops2 transAgg -d sample.all -loops sample.fixed_trans_loops.txt \
...
Pair statistics use `Npair`, cross-sample RPM uses validated `Lglobal`, and
`Unique PETs` remains the physical output-row count.
...
Trans differential analysis counts an independently fixed candidate family in
each raw biological sample; downsampled or pooled group counts are never used
as replicates. The sample sheet has `sample`, `condition`, and `directory`
columns:
...
Two or more biological samples per condition use an NB GLM and also produce a
runnable edgeR QL script. One sample per condition is explicitly a technical
exact test (`biological_inference=false`, `significant=NA`). Replacement data
are rejected. Raw counts, `Npair`, axis marginals and selected exposures are
all exported.
